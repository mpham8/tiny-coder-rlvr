...
class IpcWeightSyncTest(unittest.TestCase):
    def test_ipc_sleep_sync_generate_rollouts(self):
        ...
        with tempfile.TemporaryDirectory(prefix="hf-vllm-ipc-") as tmp:
            intermediate = Path(tmp)

            t0 = time.perf_counter()
            policy.save(intermediate)
            seed_save_s = time.perf_counter() - t0
            ...
            try:
                ...
                t0 = time.perf_counter()
                # Bind while HF stays on CPU — init only ships the packed flag.
                ...
                # Train half: discard vLLM VRAM, run a tiny HF step, then IPC.
                t0 = time.perf_counter()
                ...
                # Fake adam allocation path (state created on first real step).
                ...
                print(
                    ...
                    f"seed save:     {seed_save_s:6.2f}s\n"
                    ...
                    f"total:         {hf_load_s + seed_save_s + vllm_load_s + init_sync_s + generate_v1_s + sleep_s + train_s + wake_sync_s + generate_v2_s:6.2f}s"
                )
            finally:
                generator.shutdown()
                policy.to_cpu(optimizer)
